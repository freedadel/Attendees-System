  ...
  <div class="page" style="background-image:url({{asset('/img/report3.jpg')}});background-size: 100%;margin-top:0px">
    <div class="subpage">
      <div class="row col-12" style="direction: rtl;margin-top:10px">
        ...
        <div class="col-8" style="display: inline-block"><h3 style="text-align: center">تقرير التوزيع</h3>
        ...
        <div class="col-4 pt-1 pb-1">
          <div class="card col-11" style="margin: auto;height:100px">
            <div class="card-header" style="background-color: #55a7c2 !important;color:#fff;">
              <h6 class="text-center">عدد المستفيدين</h5>
            </div>
            <div class="card-body">
              <h4 class="text-center">{{$orders->sum('customers')}}</h4>
            </div>
          </div>
        </div>
        @foreach ($items as $item)
        @if($item->itemsInStock->where('ddate','>=',$dateFrom)->where('ddate','<=',$dateTo)->sum('out') > 0)
        <div class="col-4 pt-1 pb-1">
          <div class="card col-11" style="margin: auto;height:100px">
            <div class="card-header" style="background-color: #55a7c2 !important;color:#fff;">
            ...
            <div class="card-body">
              ...
              <h4 class="text-center">{{$item->itemsInStock->where('ddate','>=',$dateFrom)->where('ddate','<=',$dateTo)->sum('out')}}</h4>
              @else
              <h4 class="text-center">{{$item->itemsInStock->where('ddate','>=',$dateFrom)->where('ddate','<=',$dateTo)->sum('out') * $item->price}} <span style="font-size: small">ريال</span></h4>
              @endif
            </div>
          </div>
        </div>
